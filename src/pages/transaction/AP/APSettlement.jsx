...
const APSettlement = () => {
  ...
  useEffect(() => {
    const selectedObject = getMyReceiptList.find(
      (item) => item.DocNo === receiptList
    );
  ...
  const getReceiptList = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/apreceiptlistu`
      );
  ...
  const getARDetail = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/apsettlement/${receiptList}`
      );
  ...
  const submitClick = async (e) => {
    e.preventDefault();
    if (difference !== 0) {
      return toast.error("Masih ada difference.", {
        position: "top-center",
        autoClose: 3000,
    ...
    try {
      await axios.post(`${process.env.REACT_APP_API_BASE_URL}/apsettlement`, {
        series: seriesVal,
        docDate: docDate,
  ...
  return (
    <div>
      <div className="flex justify-between">
        <div className="text-2xl font-bold mb-4">AP Settlement</div>
      </div>
