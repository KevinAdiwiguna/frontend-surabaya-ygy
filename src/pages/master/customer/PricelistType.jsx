...
export const PricelistType = () => {
  ...
  const deleteData = async (params) => {
    try {
      await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/pricelist/${params}`
      );
      dataFetching();
  ...
  const submitClick = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${process.env.REACT_APP_API_BASE_URL}/pricelist`, {
        code: code,
        name: name,
  ...
  return (
    <div>
      <div className="text-2xl font-bold mb-4">Pricelist Type</div>
