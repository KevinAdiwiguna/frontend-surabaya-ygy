...
export const MaterialGroup2 = () => {
  const { fetchMe, response } = useMe();
  const [group1, setGroup1] = useState([]);
  const [group1val, setGroupVal] = useState("");
  ...
  const dataFetching = async () => {
    try {
      const data = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/materialgroup2`);
  ...
  const submitClick = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${process.env.REACT_APP_API_BASE_URL}/materialgroup2`, {
        group1: group1val,
        code: code,
        name: name,
  ...
  const updateData = async (params) => {
    try {
      await axios.patch(`${process.env.REACT_APP_API_BASE_URL}/materialgroup2/${params}`, {
        name: nameUpdate,
        changedBy: response.User,
      });
      dataFetching();
  ...
  const deleteData = async (params) => {
    try {
      await axios.delete(`${process.env.REACT_APP_API_BASE_URL}/materialgroup2/${params}`);
      dataFetching();
  ...
  return (
    <div>
      <div className="text-2xl font-bold mb-4">Material Group 2</div>
      <form onSubmit={submitClick}>
        <table className="border-separate border-spacing-2 w-1/2">
          <tr>
            <td className="text-right">Group 1:</td>
            <td>
              <select onChange={(e) => setGroupVal(e.target.value)} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-[30%] p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
          ...
          <tr>
            ...
            <td>
              <input type="text" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-[20%] p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Isi kode" onChange={(e) => setCode(e.target.value)} required />
            </td>
          </tr>
          <tr>
            <td className="text-right">Name: </td>
            <td>
              <input type="text" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Isi nama" onChange={(e) => setName(e.target.value)} required />
            </td>
          </tr>
          <tr>
            <td></td>
