...
export const MaterialGroup3 = () => {
  ...
  const dataFetching = async () => {
    try {
      const data = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/materialgroup3`);
  ...
  const submitClick = async (e) => {
    e.preventDefault();
    try {
      const postData = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/materialgroup3`, {
        group1: group1val,
        group2: group2val,
        code: code,
        name: name,
  ...
  const deleteData = async (params) => {
    try {
      await axios.delete(`${process.env.REACT_APP_API_BASE_URL}/materialgroup3/${params}`);
      dataFetching();
  ...
  const updateData = async (params) => {
    try {
      await axios.patch(`${process.env.REACT_APP_API_BASE_URL}/materialgroup3/${params}`, {
        name: nameUpdate,
        changedBy: response.User,
      });
      dataFetching();
      ...
      <form onSubmit={submitClick}>
        <table className="border-separate border-spacing-2 w-1/2">
          <tr>
            ...
            <td>
              <select onChange={(e) => setGroup1Val(e.target.value)} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-[30%] p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
          ...
          <tr>
            ...
            <td>
              <select onChange={(e) => setGroup2Val(e.target.value)} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-[30%] p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                <option disabled selected hidden>
                  Pilih group 2
                </option>
                {group2.map((res, key) => {
