...
const CashierPayment = () => {
    ...
    const [getMyReceiptList,setGetReceiptList] = useState([])
    ...
    useEffect(()=>{
        const selectedObject = getMyReceiptList.find(item => item.DocNo === receiptList)
        setSupplierCode(selectedObject?.SupplierCode)
      },[receiptList])
    ...
    const getReceiptList = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/apreceiptlistcreated`);
        setGetReceiptList(response.data);
      } catch (error) {
        console.log(error);
      }
    };
    ...
    const getSeries = async () => {
      try {      
        const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/seriescode/CASHIER PAYMENT`);
    ...
    const submitClick = async (e) => {
      e.preventDefault()
      try {
        await axios.post(`${process.env.REACT_APP_API_BASE_URL}/cashierrpayment`, {
          generateDocDate: generateDocDate(),
          series: seriesVal,
          docDate,
          apRecListNo: receiptList,
          supplierCode,
          ...
          cashierPaymentGArray: giroDetail,
          cashierPaymentDArray: detail,
        })
    ...
    const handleGiroDetail = () => {
      const giroLainCoy = {supplierCode,bank,giroNo,giroDetailCurrency,dueDate,giroValue,exchangeRate,giroValueLocal}
    ...
    return (
      <div>
        <div className="flex justify-between">
          <div className="text-2xl font-bold mb-4">Cashier Payment</div>
        </div>
          ...
          <div className="w-full">
            <div className="flex justify-start items-start flex-col">
              <table className="border-separate border-spacing-2 ">
                ...
                <tr>
                  <td className="text-right">Receipt List: </td>
                  <td>
                    <select
                      required
                      onChange={(e) => setReceiptList(e.target.value)}
                      className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    >
                      <option value="" disabled selected hidden>
                        Pilih Request List
                      </option>
                      {getMyReceiptList.map((res, key) => {
                        return (
